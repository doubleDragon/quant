...
class PublicClient:
    ...
    def depth(self, symbol):
        ...
        resp = self._get(self.url_for(path=path, parameters=parameters))
        if resp is not None:
            data = {
                u'bids': [],
                u'asks': []
...
class PrivateClient(PublicClient):
    ...
    def get_order(self, symbol, order_id):
        url = self.url_for('order_info.do')
        params = {
            u'symbol': str(symbol),
            u'order_id': str(order_id)
        }
        resp = self._post(url=url, params=params)
        if resp is not None:
            tmp = None
            for item in resp[u'orders']:
                if item[u'order_id'] == order_id:
                    tmp = item
            if tmp is None:
                return None
            data = {
                u'order_id': tmp[u'order_id'],
                u'price': tmp[u'price'],
                u'status': constant.get_status(constant.EX_OKEX, tmp[u'status']),
                u'type': tmp[u'type'],
                u'amount': Decimal(tmp[u'amount']),
                u'deal_amount': Decimal(tmp[u'deal_amount'])
            }
            return data
    ...
    def cancel_order(self, symbol, order_id):
        ...
        撤消单个订单
        """
        url = self.url_for('cancel_order.do')
        params = {
            u'symbol': str(symbol),
            u'order_id': str(order_id)
        }
